...
import db
from routes.tasks import router as tasks_router, set_delay
...
SERVICE_FAILURE = Gauge(
    "service_simulated_failure", "Whether service is in simulated failure mode", ["service"],
)
SERVICE_FAILURE.labels(service="task-service").set(0)

_SERVICE_SIMULATED_DOWN = False

app = FastAPI(title="AutoHeal AI — Task Service", version="1.0.0")
...
async def metrics_middleware(request: Request, call_next):
    ...
    if not path.startswith("/metrics"):
        REQUESTS_TOTAL.labels(
            service="task-service", method=request.method,
            path=path, status_code=str(response.status_code),
        ).inc()
        REQUEST_LATENCY.labels(service="task-service", path=path).observe(elapsed)
    return response
...
async def health(request: Request):
    ...
    return JSONResponse(
        status_code=200,
        content={"status": "ok", "service": "task-service", "ts": datetime.now(timezone.utc).isoformat()},
...
async def metrics():
    if db.PRIMARY_POOL:
        DB_POOL_SIZE.labels(service="task-service").set(db.PRIMARY_POOL.get_size())
        DB_POOL_AVAILABLE.labels(service="task-service").set(db.PRIMARY_POOL.get_idle_size())
    SERVICE_FAILURE.labels(service="task-service").set(1 if await db.is_db_simulated_down() else 0)
...
async def startup():
    await db.connect_db()
    logger.info("task_service_started", port=settings.service_port)


@app.on_event("shutdown")
async def shutdown():
    await db.disconnect_db()
    logger.info("task_service_shutdown_complete")
