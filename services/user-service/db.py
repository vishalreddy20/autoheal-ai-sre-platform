"""
asyncpg connection pool management for user-service.
Supports primary + replica pools with exponential backoff retry.
"""
import asyncio
import asyncpg
import structlog
...
# DB simulation flag (protected by lock)
...
async def _create_pool(dsn: str, service_name: str, attempt_label: str) -> asyncpg.Pool:
    max_attempts = 5
    delay = 1.0
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            logger.info("pool_created", label=attempt_label, attempt=attempt)
            return pool
            ...
            if attempt == max_attempts:
                logger.error("pool_creation_exhausted", label=attempt_label)
                raise
...
async def connect_db() -> None:
    ...
    PRIMARY_POOL = await _create_pool(
        settings.supabase_db_url, settings.service_name, "primary"
    )
    REPLICA_POOL = await _create_pool(
        settings.replica_url, settings.service_name, "replica"
    )
