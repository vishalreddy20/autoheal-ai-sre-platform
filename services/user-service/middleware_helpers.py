"""
Shared middleware helpers for user-service and task-service.
Avoids code duplication across services.
"""
import uuid
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    """Injects X-Request-ID and X-Trace-ID into every request and response."""
...
def setup_tracing(app, service_name: str, jaeger_endpoint: str) -> None:
    if not _OTEL_AVAILABLE:
        logger.warning("otel_not_available", service=service_name)
        return
    try:
